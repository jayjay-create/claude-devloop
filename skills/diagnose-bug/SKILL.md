...
**A refusal is not a red signal.** A command blocked before it ran, or one that
came back with a message in place of output, exits non-zero exactly like a real
failure and has said nothing about the bug. Check that the red comes from the
code under test before spending step 2 on it, and report the refusal the way the
block above says.
